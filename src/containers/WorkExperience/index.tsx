import { Title } from "../../components";
import Card from "../../components/Card";

const WorkExperience = () => {
  return (
    <section>
      <Title>Experiência profissional</Title>
      <Card title="Freelancer">
        Desenvolvedor Freelancer com paixão pela criação de projetos inovadores
        e que agregam para as pessoas, sou apaixonado pela criação de interfaces
        interativas e visualmente agradáveis para o público. Fazendo projetos
        para clientes e para conhecimentos técnico, cada projeto eu aprendo cada
        vez mais a tecnologia utilizada. Procuro a minha primeira experiência
        profissional na área, portanto, estudo todos os dias para isso! {":)"}
      </Card>
    </section>
  );
};

export default WorkExperience;
